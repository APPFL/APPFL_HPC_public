...
def train_executor_ds(
    ...
    local_rank_vars = ['MPI_LOCALRANKID', 'PALS_LOCAL_RANKID', 'PMIX_LOCAL_RANK', 'SLURM_LOCALID']
    ...
    try:
        ...
        # time.sleep(random.randint(0, 40)) # spread the load/call to extract model in high GPU count
        # donwload_model_start_time = time.time()
        # if model is not None:
        #     model = load_global_model(client_agent_config, model)
        # total_model_download_time = time.time() - donwload_model_start_time
        ...
        if model is not None:
            if global_rank == 0:
                ...
                print(f"Rank 0: Model download completed in {total_model_download_time:.2f}s")
        ...
        if model is not None:
            if global_rank == 0:
                ...
                client_agent.load_parameters(model)
                print(f"Rank 0: Parameters loaded, broadcasting to {world_size - 1} other ranks...")
        ...
        if client_agent_config.ds_configs.zero_stages == 3:
            with deepspeed.zero.GatheredParameters(client_agent.trainer.model_engine.module.parameters(), modifier_rank=0):
                if global_rank == 0:
        ...
        if global_rank == 0:
            ...
            model_save_path = os.path.join(client_agent_config.save_dir, f"round{meta_data['round']}.pth")
            torch.save(local_model, model_save_path)
            # local_model = client_agent.get_parameters() # if need val_results, replace local model returned with the full local model
            if isinstance(local_model, tuple):
                local_model, meta_data_local = local_model
            else:
                meta_data_local = {}
            ...
            if client_agent_config.train_configs.mode == "epoch":
                meta_data_local['avg_epoch_loss'] = client_agent.trainer.avg_epoch_loss_dict
